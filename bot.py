...
def handle_message(message):
    ...
    if msg == '/test':
        ...
        ids = re.compile(r'[A-Za-z]+[-][0-9]+').findall(msg)
        if not ids: send_msg('消息似乎不存在符合<b>“字母-数字”</b>格式的番号，请重试或使用<b>“/av 番号”</b>进行查找 =_=')
        else:
            ids = set(ids)
            ids_msg = ', '.join(ids)
...
def help():
    '''发送指令帮助消息'''
    msg = '''发送给机器人一条含有番号的消息，机器人会匹配并搜索消息中所有符合<b>“字母-数字”</b>格式的番号（其它格式的番号可通过<code>/av</code>命令查找）。如果搜索到结果，将返回番号对应AV的<b>封面，标题，日期，演员，磁链</b>等**
